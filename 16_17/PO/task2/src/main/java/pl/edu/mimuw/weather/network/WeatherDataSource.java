...
import pl.edu.mimuw.weather.controller.WeatherAppController;
...
public abstract class WeatherDataSource {
    ...
    private static final int DEFAULT_POLL_INTERVAL = WeatherAppController.DEFAULT_POLL_INTERVAL;
    private static final int INITIAL_DELAY = 3;
    private static final int TIMEOUT = 20;

    public Observable <? extends WeatherAppEvent> dataSourceStream () {
        return dataSourceStream(DEFAULT_POLL_INTERVAL);
    }

    public Observable <? extends WeatherAppEvent> dataSourceStream (Integer pollInterval) {
        /*
    ...
    protected abstract Observable <? extends WeatherAppEvent> makeRequest ();
    ...
    private <T> Observable <WeatherAppEvent> wrapRequest (Observable <T> observable) {
		/*
        ...
        return observable.flatMap(ignore -> Observable.concat(Observable.just(new NetworkRequestIssuedEvent()), // emit
                ...
                makeRequest().timeout(TIMEOUT, TimeUnit.SECONDS).doOnError(log::error) // if
                        ...
                        // abort
                        // it
                        .cast(WeatherAppEvent.class).onErrorReturn(ErrorEvent::new), // and
                // emit
                // an
                // error
                // event
