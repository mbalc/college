...
public class EventStream {
	...
	private CompositeObservable<WeatherAppEvent> composite = new CompositeObservable<>();

	public static EventStream eventStream() {
		return instance;
	}

	public Observable<WeatherAppEvent> events() {
		return eventStream().composite.toObservable();
	}

	@SuppressWarnings("unchecked")
	public static Subscription joinStream(Observable<? extends WeatherAppEvent> observable) {
		final CompositeObservable<WeatherAppEvent> local = eventStream().composite;
		synchronized (local) {
			return local.addAll((Observable<WeatherAppEvent>) observable);
		}
	}

	public static Subscription joinBackpressuredStream(Observable<? extends WeatherAppEvent> observable) {
	...
	public Observable<WeatherAppEvent> eventsInFx() {
		return events().compose(fxTransformer());
	}

	public Observable<WeatherAppEvent> eventsInIO() {
